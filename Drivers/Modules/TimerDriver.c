...
void TimerDriver_init(void){
	...
	TIM2->ARR = (50 - 1);		// Her 100 saymada bir başa dönsün, her 2.5ms de bir interrupt tetiklenecek
...
void TIM2_IRQHandler(void){
	...
	if(TIM2->SR & (1UL << 0)){	// TIM2->SR ın 0. biti UIF flag i dir. Bu set olmuşsa interrupt olmuş demek olur
		...
		Stepper_HalfDriveMode(counter);
		counter++;

		if(counter == 2048){			// HALF Drive için yarım tur demek
			Stepper_ToggleDirection();
			GPIOA->ODR ^= (1UL << 5);	// LED aç kapa
			counter = 0;				// Counter reset
			Stepper_SetSpeed(20);		// TIM2 ARR değiştirerek motor hızının değiştirilmesi
		}
